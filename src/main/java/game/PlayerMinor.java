...
public class PlayerMinor {
    ...
    public static double k[][] = new double[][] {{0.7, 1.4, 0.2}, {1.0, 1.5, 0.4}, {1.0, 1.5, 0.4}};
    ...
    public static double valueFor(GameState gameState, int x, int y, int[] z, int emptyCells) {
        ...
        for (int i = 0; i < gameState.opponentCount; i++) {
            minZ = Math.min(minZ, Math.pow((z[i] == 0 ? 0.5 : z[i]) * 1.0 / (x == 0 ? 0.5 : x), k3 / 20 + k3 * 19 *
                    (emptyCells * 1.0 / N / M) / 20.0));
        }
        return value * minZ;
    }
